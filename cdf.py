...
def cdf(var, weights=None, lower=None, upper=None, bins=None):
    ...
    if bins is not None:
        ...
        if upper is None:
            upper = np.max(var)
        cdf, bin_edges = np.histogram(
            var, weights=weights, bins=bins, range=[lower, upper]
        )
...
def percentile_from_cdf(cdf_x, cdf, percentile):
    ...
    assert (percentile >= cdf[0]).all() and (
        percentile <= cdf[-1]
    ).all(), "Percentile {} must be in cdf bounds".format(percentile)
